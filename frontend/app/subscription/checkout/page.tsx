...
export default function SubscriptionCheckoutPage() {
  ...
  useEffect(() => {
    ...
    if (savedPlan) {
      ...
      toast.error("Tidak ada paket langganan yang dipilih.");
  ...
  const selectSavedAddress = (addr: any) => {
    ...
    toast.success("Alamat tersimpan dimuat.");
  };

  const handleCheckout = async () => {
    if (!address.area_id || !address.address || !shippingData.name) {
      toast.error("Mohon lengkapi alamat dan identitas penerima.");
      return;
    }
  ...
  if (loading || !plan) return (
    <div className="min-h-screen bg-[#FAF9F6] flex flex-col items-center justify-center gap-4 text-stone-400 font-sans">
      ...
      <p className="text-[10px] font-black uppercase tracking-[0.3em]">Mempersiapkan Checkout...</p>
    </div>
  );
            ...
            <div className="lg:col-span-8 space-y-10">
               ...
               <div className="bg-stone-50 border border-black/5 p-8 rounded-sm flex gap-4 items-start shadow-inner">
                  ...
                  <div className="space-y-1">
                     ...
                     <p className="text-[10px] font-bold text-stone-500 leading-relaxed italic">Paket langganan Anda akan selalu diproses pada hari pertama roasting setiap bulannya untuk menjamin kesegaran maksimal.</p>
                  </div>
               </div>
            </div>
                  ...
                  <div className="bg-white p-10 border border-black/10 shadow-[12px_12px_0_rgba(0,0,0,0.03)] rounded-sm space-y-10 relative overflow-hidden">
                    ...
                    <div className="space-y-6">
                      <div className="flex justify-between items-end">
                        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Paket <br/><span className="italic font-display text-sm text-slate-900">{plan.name}</span></p>
                      ...
                      <div className="flex justify-between items-end">
                        ...
                        <span className="font-bold italic font-sans text-sm text-emerald-500">GRATIS</span>
                      </div>
                      ...
                      >
                        {processing ? (
                            ...
                            <span className="flex items-center gap-3">Konfirmasi & Bayar <CreditCard size={16} /></span>
                        )}
                      </Button>
                    </div>
                    ...
                    <div className="mt-6 p-5 bg-[#367F4D]/[0.03] border border-[#367F4D]/10 rounded-sm">
                      <p className="text-[8px] font-bold text-stone-400 uppercase tracking-widest text-center italic leading-relaxed">
                        Pembayaran akan diproses aman melalui <span className="text-slate-900">Xendit Payment Gateway</span>. Anda menyetujui syarat & ketentuan berlangganan.
                      </p>
                    </div>
                  </div>
            </div>
