...
import { Check, Calendar, Package, Sparkles, ArrowRight } from "lucide-react";
             ...
             <div key={plan.name} className="bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-sm flex flex-col hover:shadow-xl transition-all duration-500 hover:-translate-y-2 group">
                <div className="flex-1 space-y-6">
                  <div className="space-y-2">
                    ...
                    <p className="text-2xl font-mono font-bold text-slate-800">{plan.price}<span className="text-xs text-slate-400 font-sans tracking-normal">/month</span></p>
                  </div>
                ...
                <Button className={`mt-10 w-full h-14 rounded-2xl font-black tracking-widest text-[10px] uppercase transition-all duration-500 ${plan.color} text-white group-hover:scale-[1.02]`}>
                   Subscribe Now
                </Button>
             </div>
           ))}
