...
const planVisuals: any = {
  "The Discovery": { icon: <Sprout size={24} className="text-[#D88C8C] mb-4" />, color: "bg-[#D88C8C] text-white", hoverColor: "hover:bg-[#C27A7A]", popular: false },
  "Master's Choice": { icon: <Star size={24} className="text-[#F1B941] mb-4" />, color: "bg-[#2A1619] text-white", hoverColor: "hover:bg-black", popular: true },
  "The Collector": { icon: <FlaskConical size={24} className="text-[#D88C8C] mb-4" />, color: "bg-[#5C1B1B] text-white", hoverColor: "hover:bg-[#4A1616]", popular: false }
};
    ...
    <div className="bg-[#FAF9F6] min-h-screen relative overflow-hidden font-sans">
      ...
      >
        <div className="max-w-7xl mx-auto mt-12 grid grid-cols-1 lg:grid-cols-3 gap-12 lg:gap-16">
          {plans.map((plan, i) => {
            const visual = planVisuals[plan.name] || { icon: <Sprout size={24} className="text-[#D88C8C] mb-4"/>, color: "bg-stone-900 text-white", hoverColor: "hover:bg-black", popular: false };
            return (
                <div key={plan.name} className={`plan-card relative bg-white p-10 border border-black/10 flex flex-col transition-all duration-500 group shadow-[10px_10px_0px_rgba(216,140,140,0.03)] hover:shadow-[12px_12px_0px_rgba(216,140,140,0.05)] hover:-translate-y-1 ${visual.popular ? 'border-[#D88C8C]/40 -translate-y-4 lg:z-20' : 'lg:z-10'}`} style={{ transform: `rotate(${i === 1 ? 0 : (i === 0 ? -1 : 1)}deg)` }}>
                  ...
                  {visual.popular && <Sticker rotate={-6} className="absolute -top-8 -right-6 z-40 border border-black/5 shadow-sm" color="#F1B941">MASTER'S PICK</Sticker>}
                  <div className="flex-1 space-y-8">
                    <div className="text-center space-y-4">
                       {visual.icon}
                       <h3 className="text-3xl font-cloude text-[#2A1619]">{plan.name}</h3>
                       <div className="flex items-baseline justify-center gap-1">
                         <span className="text-4xl font-bold text-slate-900 leading-none">Rp {Number(plan.price).toLocaleString('id-ID')}</span>
                         <span className="text-[11px] font-black uppercase text-stone-400">/Bln</span>
                       </div>
                    </div>
                    ...
                    <p className="text-sm text-stone-500 font-medium leading-relaxed text-center italic bg-stone-50 p-4 rounded-lg border border-black/5">"{plan.description}"</p>
                  ...
                  <button onClick={() => handleInitSubscribe(plan)} className={`mt-10 w-full h-16 rounded-xl font-black tracking-[0.3em] text-[10px] uppercase transition-all duration-300 shadow-sm flex items-center justify-center gap-3 hover:-translate-y-1 active:scale-95 ${visual.color} ${visual.hoverColor}`}>
