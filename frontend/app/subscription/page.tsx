...
export default function SubscriptionPageV2() {
  ...
  const steps = [
    ...
    { title: tSub.step2Title, desc: tSub.step2Desc },
    { title: tSub.step3Title, desc: tSub.step3Desc }
  ];
  ...
  // 1. Fetch data API di awal mount komponen (Biarkan tetap seperti ini)
  useEffect(() => {
    setMounted(true);
  ...
  useEffect(() => {
    ...
    let ctx = gsap.context(() => {
      ...
      // B. Animasi Master Roaster Section
      if (masterRef.current) {
        const polaroid = gsap.utils.toArray(".master-polaroid", masterRef.current);
        if (polaroid.length > 0) {
          gsap.fromTo(polaroid, { x: -100, rotation: -10, opacity: 0 }, { x: 0, rotation: -3, opacity: 1, duration: 1.2, ease: "back.out(1.2)", scrollTrigger: { trigger: masterRef.current, start: "top 70%" } });
        }
        const quote = gsap.utils.toArray(".master-quote", masterRef.current);
        if (quote.length > 0) {
          gsap.fromTo(quote, { x: 50, opacity: 0 }, { x: 0, opacity: 1, duration: 1, ease: "power2.out", scrollTrigger: { trigger: masterRef.current, start: "top 70%" } });
        }
      }

      // C. Animasi Steps Note Layout
      if (stepsRef.current) {
        const steps = gsap.utils.toArray(".step-note", stepsRef.current);
        if (steps.length > 0) {
          gsap.fromTo(steps, { y: 50, opacity: 0 }, { y: 0, opacity: 1, rotation: (i) => i % 2 === 0 ? -0.5 : 0.5, stagger: 0.2, duration: 1, ease: "power2.out", scrollTrigger: { trigger: stepsRef.current, start: "top 75%" } });
        }
      }
    });
    ...
    const timer = setTimeout(() => {
      ctx = gsap.context(() => {
        if (pricingRef.current) {
          const cards = gsap.utils.toArray(".plan-card", pricingRef.current);
          if (cards.length > 0) {
            gsap.fromTo(cards, 
              { y: 40, opacity: 0 }, 
              {
  ...
  }, [mounted, plans.length]);

  const handleInitSubscribe = (plan: any) => {
    if (!user) {
      toast.error(tSub.loginRequired);
          ...
          {plans.map((plan, i) => {
            ...
            return (
              <div key={plan.id} className={`plan-card relative bg-white p-10 border border-black/10 flex flex-col transition-all duration-500 group shadow-lg shadow-black/5 hover:shadow-2xl hover:shadow-black/10 hover:-translate-y-2 ${visual.popular ? 'border-[#F1B941]/40 -translate-y-4 lg:z-20' : 'lg:z-10'}`}>
                ...
                {visual.popular && <Sticker rotate={0} className="absolute -top-8 -right-4 z-40 border border-black/10 shadow-md" color="#F1B941">{tSub.mastersPick}</Sticker>}
                ...
                <button onClick={() => handleInitSubscribe(plan)} className={`mt-10 w-full h-16 font-black tracking-[0.3em] text-[10px] uppercase transition-all duration-300 shadow-lg flex items-center justify-center gap-3 hover:-translate-y-1 active:scale-95 ${visual.color} ${visual.hoverColor}`}>
                  {tSub.subscribeNow} <ArrowRight size={14} strokeWidth={3} />
                </button>
              </div>
            );
          })}
