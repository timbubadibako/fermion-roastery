...
export default function SubscriptionPageV2() {
  ...
  // 2. 🟢 SATU CONTEXT UTAMA: Handle Animasi Statis & Dinamis Sekaligus Tanpa Lock
  useEffect(() => {
    if (!mounted) return;

    let ctx: gsap.Context;

    // Delay 60ms agar DOM dari list 'plans' dipastikan sudah disuntikkan oleh React
    const timer = setTimeout(() => {
      ctx = gsap.context(() => {

        // A. Animasi Hero Text (Statis)
        ...
        // B. Animasi Master Roaster Section (Statis)
        ...
        // C. Animasi Steps Note Layout (Statis)
        if (stepsRef.current) {
        ...
        // D. Animasi Pricing Cards (Dinamis - Hanya berjalan jika data plans sudah ada)
        ...
        // Segarkan kalkulasi detektor scroll layout
    ...
    return () => {
      clearTimeout(timer);
      if (ctx) ctx.revert(); // Bersihkan context lama agar tidak double-render pas plans masuk
    };
  }, [mounted, plans.length]); // Mengawasi 'plans.length' agar ketika array terisi, context langsung dihitung ulang dengan bersih
