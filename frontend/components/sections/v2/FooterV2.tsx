...
export function FooterV2() {
  ...
  return (
    <footer className="bg-slate-950 pt-20 pb-12 px-6 relative overflow-hidden font-sans">
      {/* Top Border Glow */}
      <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-fermion-french-blue/50 to-transparent" />
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-end gap-12 relative z-10">
        ...
        <div className="space-y-8 text-left">
           <h2 className="text-5xl font-display font-black text-white italic leading-none">
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-fermion-french-blue to-purple-400 not-italic font-sans">{content.titleSub}</span>
           </h2>
           
           <div className="flex flex-wrap gap-3">
              {content.socials.map((social, idx) => (
                <div key={idx} className="p-1 bg-white rotate-[-3deg] shadow-lg even:rotate-[5deg]">
                   <div className="border border-dashed border-slate-200 px-3 py-1 text-[8px] font-black uppercase tracking-widest text-slate-400 cursor-pointer hover:text-fermion-french-blue transition-colors">
                     {social}
                   </div>
                </div>
              ))}
           ...
           <div className="space-y-4">
              ...
              <ul className="space-y-2 text-white text-xs font-black uppercase italic tracking-widest">
                 {content.navLinks.map((link, idx) => (
                   <li key={idx} className="hover:text-fermion-french-blue transition-colors cursor-pointer">{link}</li>
                 ))}
              </ul>
           </div>
        </div>
