...
// 🟢 STRUKTUR BARU ADMIN: Ramping, Fungsional, Tanpa Dead-links / Gimmick
const adminMenu = [
  {
    group: "IKHTISAR",
    items: [
      { id: "overview", label: "Ringkasan Operasi", href: "/admin", icon: LayoutDashboard },
    ]
  },
  {
    group: "REVENUE & SAAS",
    items: [
      { id: "subscriptions", label: "Paket Langganan", href: "/admin/subscriptions", icon: Layers },
    ...
    <aside className="w-64 bg-slate-950 h-screen fixed left-0 top-0 text-white flex flex-col z-50 border-r border-white/5">
      ...
      <div className="h-24 flex flex-col justify-center px-8 border-b border-white/5">
        ...
        <p className="text-[8px] font-bold text-slate-500 tracking-[0.4em] uppercase mt-2">
          {role === "ADMIN" ? "Command Center" : "Portal Partner"}
        </p>
      </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <item.icon size={18} strokeWidth={isActive ? 2.5 : 2} className={isActive ? "text-[#367F4D]" : "text-slate-400 group-hover:text-white"} />
      ...
      <div className="p-4 border-t border-white/5 space-y-4">
        ...
        <div className="flex items-center gap-3 px-4 py-2 bg-white/5 rounded-2xl">
          <div className="w-8 h-8 rounded-lg bg-[#367F4D] flex items-center justify-center font-bold text-xs text-white">
            {user?.full_name?.substring(0, 2).toUpperCase() || "LC"}
          </div>
          <div className="flex-1 overflow-hidden text-left">
            <p className="text-[10px] font-bold truncate">{user?.full_name || "Local Dev"}</p>
